...
brn_imp_to_inspect = finch.brn_.impl.brn_blocked_cpp
...
# start scheduler
finch.start_scheduler(debug=debug)

# brn experiments
if run_brn:
    if brn_single_run:
        arrays = finch.brn_.load_input(data_file_type=brn_single_data_file_type)
        runtime = finch.measure_runtimes([brn_imp_to_inspect], [lambda : arrays], iterations=brn_single_reps)[0]
        print(runtime)
