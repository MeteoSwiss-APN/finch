...
DefaultOperator = Callable[[xr.Dataset], xr.Dataset]
"""
...
def _reduce_runtimes(
    rt: list[Runtime], reduction: Callable[[np.ndarray, int], np.ndarray]
) -> Runtime:
...
def measure_runtimes(
    ...
    **kwargs
    ...
    reportfile = util.get_path(
        config["evaluation"]["perf_report_dir"], "dask-report.html"
    )
...
def xr_run_prep(
    remove_existing_output: bool = True, clear_scheduler: bool = False
) -> dict[str, Any]:
    """
...
def xr_impl_runner(
    ...
    **kwargs
    ...
    ds = ds + xr.full_like(
        ds, random.random()
    )  # instead of clone. See https://github.com/dask/dask/issues/9621
    out = impl(ds)
...
def measure_loading_times(
    input: Input, versions: list[Input.Version], **kwargs
) -> list[float]:
    """
